...
public class DragObject2 : MonoBehaviour
{
    ...
    [Tooltip("This allows you to activate (true) or deactivate (false) 3d movement for the boiler. It is recommended you leave this off, can also be switched via button")]
    public bool move3d = false;
    private Vector3 offset;
    private Transform toDrag;
    private Transform toRotate;

    //Switches between the two Boiler movement types
    ...
    void Update()
    {
        ...
        if (Input.touchCount == 1)
        {
            ...
            if (touch.phase == TouchPhase.Began)
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    dist = Mathf.Abs(dist);                                                     //2.////////////This fixed the problem with invered movement when viewed from the back
        ...
        if(Input.touchCount == 2){
            ...
            {
                toRotate.Rotate(0f, -touch.deltaPosition.x/5, 0f);                                  //tune turning sensitivity here (changed the diving factor
            }
